...
final class ShareCardWindowController: NSWindowController {
    ...
    init(stats: UsageStats, chartTab: String) {
        self.stats = stats
        ...
        let contentView = ShareCardWindowView(stats: stats, chartTab: chartTab) {
            window.close()
        }
...
struct ShareCardWindowView: View {
    ...
    init(stats: UsageStats, chartTab: String, onClose: @escaping () -> Void) {
        self.stats = stats
    ...
    var body: some View {
        VStack(spacing: 16) {
            ...
            // 카드 미리보기
            ShareCardView(stats: stats, chartTab: currentChartTab)
    ...
    private func copyCard() {
        copyShareCardToClipboard(stats: stats, chartTab: currentChartTab)
    ...
    private func saveCard() {
        guard let image = renderShareCard(stats: stats, chartTab: currentChartTab),
